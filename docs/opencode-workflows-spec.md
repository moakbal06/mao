...
- Core plugin registry built-ins must include `@moakbal/mao-plugin-agent-opencode` under slot `agent`.
...
- Unit/integration validation that should remain green for OpenCode workflows:
  - `@moakbal/mao-plugin-agent-opencode` tests.
  - `@moakbal/mao-core` tests: `session-manager.test.ts`, `plugin-registry.test.ts`.
  - `@moakbal/mao-cli` tests: `plugins.test.ts`, `start.test.ts`, `session.test.ts`, `send.test.ts`, `status.test.ts`.
  - `@moakbal/mao-integration-tests` with `test:integration` (includes `agent-opencode.integration.test.ts`, conditionally skipped tests where prerequisites are unavailable).
