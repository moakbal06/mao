...
pnpm --filter @moakbal/mao-web typecheck    # Web only

# Testing
pnpm test                                   # All packages (excludes web)
pnpm --filter @moakbal/mao-web test         # Web tests
pnpm --filter @moakbal/mao-web test:watch   # Web watch mode
...
- `@moakbal/mao-core` for core imports
...
├── package.json          # @moakbal/mao-plugin-{slot}-{name}
...
- Package: `@moakbal/mao-plugin-{slot}-{name}` (lowercase, hyphenated)
...
- Use `normalizeAgentPermissionMode` from `@moakbal/mao-core` (not a local duplicate)
