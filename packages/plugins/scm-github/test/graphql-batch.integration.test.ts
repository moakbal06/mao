...
describe.skipIf(SKIP_INTEGRATION_TESTS)("GraphQL Batch Enrichment Integration", () => {
  const testPRs = [
    {
      ...
      number: 1,
      url: "https://github.com/moakbal06/mao/pull/1",
      title: "Test PR",
      branch: "test-branch",
      baseBranch: "main",
      isDraft: false,
  ...
  it("should enrich a single real PR", async () => {
    ...
    const enrichment = result.get("moakbal06/mao#1");
  ...
  it("should handle non-existent PR gracefully", async () => {
    const nonExistentPRs = [
      {
        ...
        repo: "agent-orchestrator",
        number: 99999999,
        url: "https://github.com/moakbal06/mao/pull/99999999",
        title: "Non-existent",
        branch: "non-existent",
        baseBranch: "main",
        isDraft: false,
    ...
    const enrichment = result.get("moakbal06/mao#99999999");
    expect(enrichment).toBeDefined();
  ...
  it("should enrich multiple PRs in a single batch", async () => {
    ...
    const multiPRs = [
      ...
      },
      {
        ...
        repo: "agent-orchestrator",
        number: 2,
        url: "https://github.com/moakbal06/mao/pull/2",
        title: "PR 2",
        branch: "branch2",
        baseBranch: "main",
        isDraft: false,
    ...
    const pr1 = result.get("moakbal06/mao#1");
    const pr2 = result.get("moakbal06/mao#2");
