...
`@moakbal/mao-core` exports two structured feedback tool contracts:
...
```ts
import { FEEDBACK_TOOL_NAMES, FeedbackReportStore, getFeedbackReportsDir } from "@moakbal/mao-core";
...
# Run specific test
pnpm --filter @moakbal/mao-core test -- session-manager.test.ts
```
...
```bash
# Build core
pnpm --filter @moakbal/mao-core build

# Typecheck
pnpm --filter @moakbal/mao-core typecheck
```
