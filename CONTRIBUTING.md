...
Open an issue at [github.com/moakbal06/mao/issues](https://github.com/moakbal06/mao/issues).
...
```bash
git clone https://github.com/moakbal06/mao.git
cd agent-orchestrator
pnpm install
pnpm build
```

Build order matters — `@moakbal/mao-core` must be built before the CLI, web, or plugins can run. `pnpm build` at the root handles this automatically.

### Running tests

```bash
pnpm test                                         # all packages
pnpm --filter @moakbal/mao-core test              # core only
pnpm --filter @moakbal/mao-core test -- --watch   # watch mode
...
pnpm --filter @moakbal/mao-web dev
```
...
`ao update` fast-forwards the local install repo, reinstalls dependencies, clean-rebuilds `@moakbal/mao-core`, `@moakbal/mao-cli`, and `@moakbal/mao-web`, refreshes the global launcher with `npm link`, and finishes with CLI smoke tests. Use `ao update --skip-smoke` when you only need the rebuild step, or `ao update --smoke-only` when validating an existing install.
...
```json
{
  "name": "@moakbal/mao-runtime-myplugin",
  "version": "0.1.0",
  "type": "module",
  "main": "dist/index.js",
...
```typescript
// src/index.ts
import type { PluginModule, Runtime } from "@moakbal/mao-core";
...
```json
"@moakbal/mao-runtime-myplugin": "workspace:*"
```
...
```bash
pnpm --filter @moakbal/mao-runtime-myplugin build
pnpm --filter @moakbal/mao-runtime-myplugin test
```
